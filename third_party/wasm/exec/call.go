...
var (
	// ErrSignatureMismatch is the error value used while trapping the VMInterpreter when
	...
	// ErrUndefinedElementIndex is the error value used while trapping the VMInterpreter when
...
func (vm *VMInterpreter) call() {
	index := vm.fetchUint32()

	vm.funcs[index].call(vm, int64(index))
}

func (vm *VMInterpreter) callIndirect() {
	index := vm.fetchUint32()
